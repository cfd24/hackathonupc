...
- **`ZSafeRWeightedYSafeVarianceAlgorithm`**: Variante original de `ZSafeRWeightedYSafeAlgorithm` con prioridad de niveles Y cercanos a X=1 y umbral de varianza basado en una foto fija de las posiciones actuales de los shuttles.
- **`Variance`**: Variante operacional de varianza. Mantiene el almacenamiento rápido de la familia `ZSafeRWeightedYSafeAlgorithm`, con prioridad de niveles Y cercanos a X=1 y respeto de `z2_start_x_ratio`. En recuperación simula la secuencia de extracción del pallet y solo exporta si la media y el máximo de las distancias cuadradas entre caja y shuttle quedan por debajo de `max_avg_squared_wagon_distance`.
